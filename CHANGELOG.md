...
- 支持每周传入一份最新同业发行明细；默认使用 `deliverables/dashboards/04_reference/2025年互联网金融ABS同业发行明细.xlsx` 作为同比基准，支持命令行覆盖。
- 固化基础资产集团、信托渠道 Top 5 + 其他、资产规模同比、AAA 周度利率和输入文件 SHA-256 追溯信息；源 Excel 不进入发布站点。

---

## v2.5.7 — 2026-07-18 机构实际规模统一改 V列中标口径

### 背景

- 业务原则明确: V列「认购份额」代表最终中标/实际持有规模；Y列「申购规模」代表申购报价规模，申购不一定中标。
- 因此投资台账、额度占用、投行认购等“实际认购/中标/持有/额度占用”统计不得使用 Y列。

### 修复

- **投资台账**:
  - `share` 固定改为 V列「认购份额」
  - `inst` 固定改为 U列「认购机构」并归一化
  - `cost` 固定改为 T列「成本」
  - W/X/Y 后续仅作为申购报价分析字段，不再复用到 `share`

- **智能问答**:
  - 明确“规模/投资规模/管理规模/认购份额”按 V列最终中标口径回答
  - 将“申购规模/报价规模/申报规模/投标规模”列为未接入口径，避免把 Y列缺失的问法误答成 V列

- **非标额度 / 授信总额度**:
  - 新增认购规模固定使用 U/V（认购机构/认购份额）
  - Y列申购规模不再用于额度占用扣减
  - 匹配审计保留，source 改为 `UV`

- **机构统计**:
  - “投行认购规模”固定使用 U列含“投行”的记录，对 V列「认购份额」求和
  - 管理规模/参与项目规模/托管规模暂保持 J列项目规模口径（已确认先按项目去重，非行复制重复）

### 验证

- 投资台账 2026 share 合计 = 1115.9175 亿，与 V列认购份额合计一致
- fig8 授信总额度新增认购 = 18.47 亿（实际中标口径）
- 综合看板端到端生成通过:13 panel + Tab JS 齐全
- `py_compile` / `node --check scripts/itl_chat.js` 通过
...
- **P1 派生指标冗余**:
  - `proj_sizes_js` 重算兜底已在 v2.5.4 前后清理，本轮复核确认 `gen_integrated_dashboard.py` 无重复计算分支

- **P2 可选面板容错一致**:
  - 投资人分析 3 个可选面板统一降级策略：理财子分析 / 非标额度 / 授信总额度异常均降级为占位块，不再由单个可选面板中断整份综合看板生成
  - 共享 tmp 创建/断言阶段新增通用异常清理，异常继续抛出但不泄漏 tmp

- **P2/P3 可维护性清理**:
  - `re` / `pandas` 局部 import 上提到模块顶部
  - 清理共享 tmp 结构断言中过时的 `iloc[2:]` 注释，改为当前单行表头 `iloc[1:]` 口径

### 验证

- `py_compile scripts/gen_integrated_dashboard.py` 通过
- 0706 定稿端到端生成 `Inbox_p123_verify.html` 通过：13 个 panel + Tab JS 齐全
