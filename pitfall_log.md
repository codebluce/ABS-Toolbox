...
- **根因**:fig4/fig5 当初假设台账是"双行表头"（Row0 主表头 + Row1 子表头），故 `iloc[2:]`。但 increment_merge 产出的定稿台账是单行表头，Row1 已是数据。代码假设与实际结构不符。
  ...
  ```
- **影响范围**:
  - fig4 理财子矩阵：少算东道17-3 邮储银行 4 亿认购（邮储银行非理财子，理财子矩阵列不受影响；但矩阵"认购份额"汇总少算）
  - fig5 理财子画像：漏的是邮储银行（非理财子），**理财子画像实际不受影响**
  - fig6/fig8 新增认购读取同样曾用 `iloc[2:]`，在当前数据下首条为 1 月记录，对 7 月起新增认购暂无影响，但为避免未来漏首条，也一并修复
- **修复方案**:`fig4_new_welizhi_matrix.load_all_rows`、`load_data.load_wxy_df`、`fig6_credit_panel`、`fig8_credit_total_panel` 的 `raw.iloc[2:]` 改为 `raw.iloc[1:]`（单行表头只跳 1 行）。
- **验证**:修复后 fig4 认购份额原始合计增加 4 亿（1125.455 → 1129.455），补回东道17-3 邮储银行 4 亿首条记录。
- **教训**:`iloc[2:]` 这类硬编码行偏移依赖台账结构假设，结构变化时静默错数。后续应进一步改用"按表头行定位"（如 `df[df['序号'].astype(str).str.isdigit()]` 筛数据行）而非固定行号。
- **状态**:**已修复**（P0 防错包）。
- **追踪**:待补 CHANGELOG
