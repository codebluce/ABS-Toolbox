...
### #ABS-006 — fig4/fig5 的 iloc[2:] 跳过台账首条认购记录

- **日期**:2026-07-16
- **严重度**:★★（单条认购漏算，金额影响小但属数据正确性问题）
- **发现者**:v2 单次预处理改造评审复核时发现（评审建议"补 Row1=WXY 子表头校验"，核查后发现 Row1 根本不是子表头）
- **现象**:fig4 `load_all_rows` 和 fig5 `load_wxy_df` 都用 `raw.iloc[2:]` 跳过前 2 行。实测 0703/0706 定稿台账结构为**单行表头**（Row0=表头，Row1 起即真实数据），`iloc[2:]` 会跳过 Row1——东道17-3 的第一条认购记录（邮储银行/4 亿）被漏掉。东道17-3 在台账共 19 行（合并单元格展开后），`iloc[2:]` 只读到 18 行。
- **根因**:fig4/fig5 当初假设台账是"双行表头"（Row0 主表头 + Row1 子表头），故 `iloc[2:]`。但 increment_merge 产出的定稿台账是单行表头，Row1 已是数据。代码假设与实际结构不符。
  ```
  Row0: ['序号','月份','类别','资产类型','项目名称',...]  <- 表头
  Row1: [1, '1月', '消金', '赊销白条', '东道17-3', ...]   <- 第1条数据(邮储银行/4亿) ← 被跳过
  Row2: [1, '1月', '消金', '赊销白条', '东道17-3', ...]   <- 第2条数据(国泰海通投行/1.3亿)
  ```
- **影响范围**:
  - fig4 理财子矩阵：少算东道17-3 邮储银行 4 亿认购（邮储银行非理财子，理财子矩阵列不受影响；但矩阵"认购份额"汇总可能少算）
  - fig5 理财子画像：漏的是邮储银行（非理财子），**理财子画像实际不受影响**
  - 其他模块（load_and_filter / fig6 / fig8）用 `iloc[2:]` 但配合 `title_mask` 过滤"成本"标题行，行为不同，不受此 bug 影响
- **修复方案**(待实施):`fig4_new_welizhi_matrix.load_all_rows` 和 `load_data.load_wxy_df` 的 `raw.iloc[2:]` 改为 `raw.iloc[1:]`（单行表头只跳 1 行）。**前置验证**：需确认所有台账（0703/0706 定稿 + 历史台账）都是单行表头，否则改成 `iloc[1:]` 会把双行表头文件的第 2 行表头当数据。
- **验证**:修复后东道17-3 应读到 19 行（含邮储银行 4 亿），fig4 矩阵认购份额合计应增加 4 亿。
- **教训**:`iloc[2:]` 这类硬编码行偏移依赖台账结构假设，结构变化时静默错数。应改用"按表头行定位"（如 `df[df['序号'].astype(str).str.isdigit()]` 筛数据行）而非固定行号。
- **状态**:**未修复**。v2 性能改造（d6cf62b）前后行为一致，未引入也未修复。作为独立议题跟进。
- **追踪**:待补 CHANGELOG
