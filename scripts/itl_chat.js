...
(function () {
  ...
  // 注：台账的规模J列是项目级数值被复制到每一行（数据加工副产物），任何逐行加总都会虚增；
  // 所有「规模/投资规模/管理规模/认购份额」类问题一律按最终中标口径V列「认购份额」合计回答（share）。
  // Y列「申购规模」只是申购报价规模，不代表最终中标；若后续支持申购报价问答，应新增独立 metric。
  ...
  var UNSUPPORTED_METRIC = /倍数|收益率|久期|余额|净值|回报|申购规模|报价规模|申报规模|投标规模/;
  ...
  function parseMetric(q) {
    ...
    if (/规模|存量/.test(q)) return 'share';   // 规模类提问统一按最终中标V列口径回答（见顶部注释）
  ...
  function handle(q) {
    ...
    if (spec.unsupported) {
      addMsg('bot', '台账里没有「' + esc(spec.unsupported) + '」这个已接入口径，我不能硬算。目前支持：认购份额/规模（均按V列最终中标口径）、平均成本、平均利差、笔数、项目数；申购报价规模需后续单独接入Y列。');
      return;
    }
    lastSpec = spec;
    var html;
