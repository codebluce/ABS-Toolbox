...
def compute_maturity_amounts():
    ...
    print(f'\n读取台账计算新增认购规模: {LEDGER_FILE}')
    tmp = preprocess_xlsx_for_pandas(LEDGER_FILE)
...
def build_html(institutions):
    """生成投资人分析看板 HTML(风格与综合看板一致)"""
    ...
    # 表格行(列序:机构 | 非标授信总额 | 新增认购规模 | 非标剩余额度(实时) | 7-12月摊还)
    rows_html = ''
    for inst in institutions:
        m = inst['maturities']
        def fmt(v):
...
<style>
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    tbody tr.totals-row td {{ border-top: 2px solid #0d1b2e; }}
    .legend {{
        margin-top: 16px;
        padding: 12px 16px;
        background: #f8f9fa;
    ...
    .legend strong {{ color: #495057; }}
</style>
</head>
<body>
<div class="dashboard">
    <div class="dashboard-header">
        <div class="dashboard-title">投资人分析</div>
        <div class="dashboard-subtitle">截至 2026-06-30 | 非标授信额度与 7-12月摊还到期额度</div>
    </div>
    <div class="panel">
        <div class="panel-title">非标授信与摊还到期额度一览</div>
        <table>
            <thead>
                <tr>
                    <th>基石投资人</th>
        ...
        <div class="legend">
            ...
            • <strong>非标剩余额度(实时)</strong>= 非标授信总额对应表格原始剩余值 - 新增认购规模(红色表示已超额度)<br/>
...
def main():
    print('=== 投资人分析看板生成 ===')
    institutions = compute_maturity_amounts()
    html_content = build_html(institutions)
