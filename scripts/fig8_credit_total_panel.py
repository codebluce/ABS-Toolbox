"""fig8_credit_total_panel.py — 总授信额度监控看板(production)

读取 20260630额度盘点.xlsx:
- 总授信 sheet: A基石投资人 / B授信总额 / C剩余额度 / D-I为7-12月摊还(待填)
- 全口径历史发行 sheet: U认购机构 / V认购份额 / X循环期结束日

算法:
  对总授信每个机构,从全口径历史发行 sheet 找 U列近似匹配 + X列在2026年N月的行,
  V列(认购份额)合计 = 该机构N月摊还到期额度

剩余额度实时更新:
  剩余额度(实时) = 表格C列值 - 新增认购规模
  新增认购规模 = 最新发行台账中,簿记时间(L列)≥2026-07-01 的同机构 V列合计(全量,不限发行场所)

输出: 总授信额度监控 panel HTML body(供综合看板集成)
"""
...
OUTPUT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                      ...
                      'lab_viz', 'fig8_credit_total_panel.html')
...
def compute_maturity_amounts(ledger_path=None, preprocessed_path=None):
    ...
    ledger_path: 发行台账路径,None 时用默认 0703 定稿
    ...
    for row in ws_credit.iter_rows(min_row=2, max_row=ws_credit.max_row, max_col=3, values_only=True):
        ...
        if name:
            institutions.append({
                ...
                'remaining': float(remaining_orig) if isinstance(remaining_orig, (int, float)) else 0.0,
                'new_subscription': 0.0,
                'maturities': {7: 0.0, 8: 0.0, 9: 0.0, 10: 0.0, 11: 0.0, 12: 0.0},
            })
    print(f'总授信机构数: {len(institutions)}')
    ...
    print(f'2026年7-12月全口径历史发行记录数: {len(hist_records)}')
...
.credit-total-table thead th:first-child { text-align: left; padding-left: 12px; }
.credit-total-table tbody td {
    padding: 8px 6px;
    text-align: center;
    border-bottom: 1px solid #e9ecef;
    color: #1d1d1f;
}
.credit-total-table tbody td.inst-name {
    text-align: left;
    padding-left: 12px;
    font-weight: 500;
    color: #0d1b2e;
...
.credit-total-table tbody tr:hover { background: #f8f9fa; }
.credit-total-table tbody tr.totals-row {
    background: #e8eef5;
    font-weight: 600;
}
.credit-total-table tbody tr.totals-row td { border-top: 2px solid #0d1b2e; }
.credit-total-legend {
    margin-top: 14px;
    padding: 10px 14px;
    background: #f8f9fa;
...
.credit-total-legend strong { color: #495057; }
"""


def render_credit_total_panel(ledger_path=None, preprocessed_path=None):
    """生成总授信额度监控 panel HTML body(供综合看板集成)
    ...
    return f'''
<div class="credit-total-dashboard">
    <div class="credit-total-header">
        <div class="credit-total-title">总授信额度监控</div>
        <div class="credit-total-subtitle">截至 2026-06-30 | 总授信额度与 7-12月摊还到期额度</div>
    </div>
    <div class="credit-total-panel">
        <table class="credit-total-table">
            <thead>
                <tr>
                    <th>基石投资人</th>
                    <th>授信总额<br/>(亿元)</th>
        ...
        <div class="credit-total-legend">
            <strong>字段说明:</strong><br/>
            • <strong>授信总额</strong>:来自 20260630额度盘点.xlsx「总授信」sheet B 列<br/>
            • <strong>新增认购规模</strong>:最新发行台账中,L列(簿记时间)≥ 2026-07-01 的同机构 V列(认购份额)合计(全量,不限发行场所)<br/>
            • <strong>剩余额度(实时)</strong>= 表格原始剩余值(C列) - 新增认购规模(红色表示已超额度)<br/>
            • <strong>7-12月摊还</strong>:该机构在「全口径历史发行」sheet 中,U列同机构(近似匹配)且 X列(循环期结束日)在 2026 年对应月份的认购份额合计<br/>
...
def main():
    """独立运行模式:生成完整 HTML 文件"""
    print('=== 总授信额度监控看板生成 ===')
    body = render_credit_total_panel()
