...
<div class="tool-banner">
  ...
  <div class="banner-title">机构成本比对工具</div>
...
<div class="container">
  ...
  <div class="footer-note">
    ...
    产品类型 = 资产类型 + 期限分类（单一期限） ·
    基准利差 = 成本 &#8722; 国股CD ·
    数据来源：{xlsx_basename} · 分析报告员 pBuddy
  </div>
</div>
...
def generate(xlsx_path, output_path=None):
    ...
    html_str = build_html(products_js, data_js, xlsx_basename)

    qc_passed = run_qc(df, dfa, products, rows, output_path, precheck_only=True)

    if qc_passed:
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(html_str)
        print(f'Done: {output_path}')
        print(f'Data rows: {len(rows)} | Products: {len(products)}')
        run_qc(df, dfa, products, rows, output_path, precheck_only=False)
    else:
        print(f'\n[ERROR] QC未通过，跳过输出文件写入（{output_path}）')
