...
- 已验证 Issue:0
- 待处理 Issue:4(v26-uv-protection r1:1 CRITICAL FAIL阻断无效 + 1 WARNING changed_files遗漏 + 1 WARNING rebook行号对齐;v30-actual-share-uv r1:1 INFO RESOLVED_SHARE_COL v26遗留不阻断);2 个待归档 slug(v28-p123-cleanup r1 APPROVED + v30-actual-share-uv r1 APPROVED 待 Agent C 归档)
...
| v30-actual-share-uv | r1 | REVIEWED / APPROVED | ✅ 4/4 | 2026-07-18 | `audit/v2.5.7-v30-actual-share-uv-r01`(✅已双推) | `b1960ec` | [A1-v30-actual-share-uv-r1.md](submissions/A1-v30-actual-share-uv-r1.md) |
...
| v30-actual-share-uv | r1 | REVIEWED / APPROVED | 1(1 INFO FUNCTION_EQUIVALENCE RESOLVED_SHARE_COL v26遗留残留,不阻断,待 C 留档) | 2026-07-18 | [B1-v30-actual-share-uv-r1.md](reviews/B1-v30-actual-share-uv-r1.md) |
...
| REV-v2.5.7-v30-actual-share-uv-r01-01 | v30-actual-share-uv | r1 | INFO | open(gen_institution_stats.py RESOLVED_SHARE_COL v26遗留残留,不在投行认购路径不影响V列口径,建议后续清理,待 C 留档,不阻断) | [B1](reviews/B1-v30-actual-share-uv-r1.md) |
...
| v30-actual-share-uv | r1 REVIEWED / APPROVED | C-close | v2.5.7 机构实际规模统一改 V列中标口径：B 审计 verdict=APPROVED 无 CRITICAL/WARNING。5 焦点独立读代码全 PASS(投资台账 share=认购份额V,RESOLVED_*_COL grep 0残留 + itl_chat.js UNSUPPORTED_METRIC 新增申购规模等,L708拒答优先于answer不误答V列 + fig6/fig8 _add_actual_subscription_columns固定U/V,Y列不扣额度 + compute_ib_subscription投行认购用V列,项目去重J列说明充分 + git show 7文件不含定价脚本P1未误改);git 核实 tag→b1960ec+7文件stat吻合无遗漏(洁净性优);5层自检层1 git+层4 py_compile PY_COMPILE_OK实测,node --check本地无node采信A+结构复核。1 项 INFO(RESOLVED_SHARE_COL v26遗留)不阻断,待 Agent C 归档 |
